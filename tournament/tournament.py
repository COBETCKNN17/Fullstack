...
def registerPlayer(name):
    ...
    conn = connect()
    c = conn.cursor()
    c.execute("INSERT INTO players(player_id, name) VALUES (%s);")
    conn.commit()
    conn.close()
...
def reportMatch(winner, loser):
    ...
    conn = connect()
    c = conn.cursor()
    c.execute("INSERT INTO results(match_id, winner, loser) \
                VALUES(%s, %s);")
    c.commit()
    c.close()
...
def playerStandings():
    ...
    conn = connect()
    c = conn.cursor()
    c.execute("SELECT name, wins FROM players ORDER BY wins DESC")
    conn.commit()
    conn.close()
...
def swissPairings():
    ...
    Returns:
      A list of tuples, each of which contains (id1, name1, id2, name2)
        ...
        name2: the second player's name
    """
    cursor.execute("SELECT a.id, a.name, a.wins, b.id, b.name, b.wins \
                    FROM players as a, players as b \
                    WHERE (a.wins = b.wins or a.wins > b.wins) and a.id != b.id \
                    ORDER BY a.wins DESC, b.wins DESC;")
