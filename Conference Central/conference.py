...
# - - - Wishlist - - - - - - - - - - - - - - - - -
    ...
    def getSessionsInWishlist(self, request):
        ...
        sessions = ndb.get_multi([ndb.Key(urlsafe=wl.sessionKey) for wl in wl_sessions])
    ...
    def deleteSessionFromWishlist(self, request):
        """deletes the session to the user's list of sessions to attend"""
        user = endpoints.get_current_user()
        if not user:
            raise endpoints.UnauthorizedException('Authorization required')
        user_id = getUserId(user)
        ...
        return self._copyWishlistSessionToForm(wishlist_session, add=False)
