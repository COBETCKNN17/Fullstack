...
ANNOUNCEMENT_TPL = ('Last chance to register! These conferences are nearly sold out: %s')
MEMCACHE_FEATURED_KEY = "FEATURED_SESSIONS"
FEATURED_TPL = "{}'s sessions: {}"
...
# - - - Session objects - - - - - - - - - - - - - - - - -
    ...
    def _createSessionObject(self, request):
        ...
        user = endpoints.get_current_user()
        if not user:
            raise endpoints.UnauthorizedException('Authorization required - Please Log In')
        user_id = getUserId(user)
    ...
    def getConferenceSessions(self, request):
        """Return all session given a conference"""
    ...
    def getConferenceSessionsByType(self, request):
        ...
        sessions = Session.query(ancestor=ndb.Key(urlsafe=request.websafeConferenceKey))
        sessionsFiltered = sessions.filter(Session.typeOfSession.upper() == request.typeOfSession.upper())
        return SessionForms(items=[self._copySessionToForm(session) for session in sessionsFiltered])
...
# - - - Wishlist - - - - - - - - - - - - - - - - -
    ...
    def addSessionToWishlist(self, request):
        """adds the session to the user's list of sessions they will attend"""
        user = endpoints.get_current_user()
        if not user:
            raise endpoints.UnauthorizedException('Authorization required')
        user_id = getUserId(user)
    ...
    def getSessionsInWishlist(self, request):
        """returns all the sessions in a conference that the user will attend"""
        user = endpoints.get_current_user()
        if not user:
            raise endpoints.UnauthorizedException('Authorization required')
        user_id = getUserId(user)
